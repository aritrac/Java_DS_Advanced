 ...
 * Description: Checks if a loop exists in the linked list, if yes, then print out the length of the loop
 */
public class GetLengthOfLoopInList {
    public static void main(String[] args) {
        ...
        if(isCyclePresent(sll)){
            System.out.println("Length of the cycle = " + getLengthOfCycle(sll));
        }
    }
    ...
    public static int getLengthOfCycle(LinkedList sll){
        ListNode slowPtr = sll.getHead(), fastPtr = sll.getHead();
        while(slowPtr != null && fastPtr.getNext().getNext() != null){ //making sure that atleast two nodes are infront of the first pointer
            fastPtr = fastPtr.getNext().getNext();                     //to avoid the null pointer exception
            slowPtr = slowPtr.getNext();
            if(slowPtr == fastPtr){
                int lengthOfCycle = 1; //set to 1 as we are incrementing the slowPtr by one below outside the loop
                slowPtr = slowPtr.getNext();
